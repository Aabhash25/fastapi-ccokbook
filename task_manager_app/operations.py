...
def read_all_tasks()=>list[TaskWithId]: #returns a list TaskwithId objects
    with open(DATABASE_FILENAME) as csvfile:
        reader = csv.DictReader(csvfile) #reads a csv where each row becomes a dictionary
        return [TaskWithId(**row) for row in reader] # It reads each line of the CSV and converts it into a Pydantic model, then returns ALL those models as a list.
...
def write_task_into_csv(task:TaskWithId):
    with open(DATABASE_FILENAME,mode="a",newline="") as file:
        writer = csv.DictWritera(file,fieldnames=column_fields)
